...
(function (global, factory) {
  ...
  function detachConfig(url, data, config) {
    ...
    const value = typeof url === 'object' ? url : { ...config,
      url,
      data
    }; // 分离请求参数
  ...
  async function handleRequest(request) {
    const params = mergeConfig(this.config, request); // 拦截后的请求参数
    ...
    config.url = buildURL(combineURL(config.baseURL, config.url), config.params); // 请求方法转大写

    config.method = (config.method || 'GET').toUpperCase(); // 调整 header 优先级
